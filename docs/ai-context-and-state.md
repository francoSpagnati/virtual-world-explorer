...
- **Visione Artificiale (OWL-ViT Ottimizzata):** Il modulo `owl_vision.py` sfrutta ora automaticamente l'accelerazione hardware della GPU (CUDA/MPS). L'inferenza è **sincrona** per evitare sfasamenti, ma l'impatto sul framerate è stato quasi totalmente abbattuto grazie all'introduzione di una **cache posizionale** (`episode_owl_cache`). Essendo la camera egocentrica e l'ambiente statico durante il singolo episodio, OWL viene invocato al massimo una volta per ogni coordinata calpestata, permettendo all'agente di esplorare e tornare sui suoi passi a costo zero.
