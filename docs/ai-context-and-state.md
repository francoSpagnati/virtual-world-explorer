...
- **Sensore Logico & Momentum:** `vision_radius` è impostata a 3.5. Quando l'agente non vede la sedia (visible=0), usa una logica di "Momentum" maggiorata (`score += 2.0`) in `main.py` per forzare l'esplorazione in linea retta ed evitare minimi locali causati dal reward shaping.
