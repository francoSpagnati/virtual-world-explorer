...
- **Agente RL (DDPG):** `agent.py` utilizza ora l'algoritmo **DDPG (Deep Deterministic Policy Gradient)**. Implementa due reti neurali: una `ContinuousPolicyNetwork` (Actor, che emette l'azione continua v, w) e un `CriticNetwork` (che valuta il valore Q dello stato-azione), supportate da un `ReplayBuffer` e reti target con aggiornamento progressivo (soft update). Grazie a questa solida architettura, l'agente apprende efficacemente e l'addestramento è stato ridotto a soli 500 episodi per dimostrazione, essendo molto più efficiente.
